  ...
  async validateDocument(): Promise<ValidationResponse> {
    ...
    try {
      ...
      if (this.failureCount >= CIRCUIT_BREAKER_THRESHOLD) {
        ...
        this.logger.warn(
          `Circuit breaker opened for GovernmentIdProvider after ${this.failureCount} failures`,
        );
      }
      throw error;
    }
  ...
  private checkCircuit(): void {
    if (this.isCircuitOpen()) {
      throw new ServiceUnavailableException(
        'GovernmentIdProvider circuit breaker is open',
      );
    }
  }
      ...
      const timer = setTimeout(() => {
        reject(
          new ServiceUnavailableException(
            `GovernmentIdProvider request timed out after ${ms}ms`,
          ),
        );
      }, ms);
