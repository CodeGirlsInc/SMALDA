...
export class ExternalValidationService {
  ...
  private async processValidationAsync(requestId: string) {
    try {
      ...
      this.logger.error(
        `Async validation processing failed for request ${requestId}: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async processValidation(requestId: string): Promise<ValidationRequest> {
    ...
    if (request.status !== ValidationStatus.PENDING) {
      throw new BadRequestException(
        `Validation request ${requestId} is not in pending status`,
      );
    }
  ...
  async findAll(
    queryDto: QueryValidationRequestDto,
  ): Promise<{ requests: ValidationRequest[]; total: number }> {
    const {
      documentId,
      validationType,
      status,
    ...
    const queryBuilder =
      this.validationRequestRepository.createQueryBuilder('validation_request');

    if (documentId) {
      queryBuilder.andWhere('validation_request.documentId = :documentId', {
        documentId,
      });
    }
    ...
    if (validationType) {
      queryBuilder.andWhere(
        'validation_request.validationType = :validationType',
        { validationType },
      );
    }
    ...
    if (requestedBy) {
      queryBuilder.andWhere('validation_request.requestedBy = :requestedBy', {
        requestedBy,
      });
    }

    queryBuilder
      .orderBy('validation_request.createdAt', 'DESC')
      .skip(offset)
      .take(limit);
  ...
  async getValidationStats(): Promise<any> {
    const stats = await this.validationRequestRepository
      ...
      .groupBy(
        'validation_request.validationType, validation_request.status, validation_request.result',
      )
      .getRawMany();
  ...
  async checkProviderHealth(): Promise<Record<string, boolean>> {
    ...
    for (const [type, provider] of this.providers.entries()) {
      try {
        ...
        this.logger.error(
          `Health check failed for provider ${type}: ${error.message}`,
        );
  ...
  async isDocumentValidated(
    ...
    const completedValidations = validationResults.filter(
      (v) =>
        ...
        v.result === ValidationResult.VALID,
    );
